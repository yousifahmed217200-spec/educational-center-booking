...
$("btnAddAssignment").addEventListener("click", async () => {
  ...
  openModal(`
    ...
    <form id="addAssignmentForm">
      ...
      <div class="form-group">
        ...
        <div class="form-hint">A teacher assignment always belongs to exactly ONE school type -- the same teacher can be assigned again separately for the other school type if needed.</div>
      </div>
      <div class="form-group">
        <label class="form-label" for="assignSubject">Subject</label>
        <select class="form-select" id="assignSubject" required disabled>
          <option value="">Select a grade and school type first</option>
        </select>
        <div class="form-hint">Only subjects linked to this grade AND this school type (Subjects tab → Link to Grades) appear here -- this keeps assignments from pointing at a subject/grade/school-type combination students can't actually see.</div>
      </div>
      <div class="form-group">
  ...
  // School Type unlocks once a grade is chosen (it's independent of grade,
  // but there's no reason to show it before the admin has picked a grade --
  // keeps the form's top-to-bottom flow logical).
  $("assignGrade").addEventListener("change", () => {
    const schoolTypeSelect = $("assignSchoolType");
    const gradeId = $("assignGrade").value;
    resetSubjectSelect();
    if (!gradeId) {
      schoolTypeSelect.innerHTML = `<option value="">Select a grade first</option>`;
  ...
  function resetSubjectSelect() {
    ...
    subjectSelect.innerHTML = `<option value="">Select a grade and school type first</option>`;
    subjectSelect.disabled = true;
  }

  // Re-populate the Subject dropdown whenever Grade OR School Type changes,
  // restricted to subjects linked to that grade with a matching school_type
  // (NULL on subject_grades = offered for both). This is what prevents an
  // orphaned assignment: a subject/grade/school-type combo students could
  // never actually reach.
  $("assignSchoolType").addEventListener("change", async () => {
    const subjectSelect = $("assignSubject");
    const gradeId = $("assignGrade").value;
    const schoolType = $("assignSchoolType").value;

    if (!gradeId || !schoolType) {
      resetSubjectSelect();
      return;
    }
    ...
    const { data, error } = await supabaseClient
      ...
      .or(`school_type.is.null,school_type.eq.${schoolType}`);
    ...
    if (subjects.length === 0) {
      subjectSelect.innerHTML = `<option value="">No subjects linked to this grade/school type yet</option>`;
      return;
    }
  ...
  $("addAssignmentForm").addEventListener("submit", async (e) => {
    ...
    if (!grade_id || !school_type || !subject_id || !teacher_id) {
      errBox.innerHTML = `<div class="banner banner-error">Please select a grade, school type, subject, and teacher.</div>`;
      return;
    }

    const { error } = await supabaseClient.from("teacher_subjects").insert({ grade_id, school_type, subject_id, teacher_id });
    if (error) {
      errBox.innerHTML = `<div class="banner banner-error">${escapeHtml(friendlyDbError(error))}</div>`;
      return;
    }
...
async function loadSlots() {
  ...
  let query = supabaseClient
    .from("lesson_slots")
    .select("id, day_of_week, start_time, end_time, capacity, active, school_type, whatsapp_group_link, grades(name), subjects(name), teachers(full_name)")
    .order("day_of_week");
  ...
  table.innerHTML = `
    <thead><tr><th>Grade</th><th>School Type</th><th>Subject</th><th>Teacher</th><th>Day</th><th>Time</th><th>Capacity</th><th>Booked</th><th>WhatsApp Group</th><th>Status</th><th>Actions</th></tr></thead>
    <tbody>${(slots || []).map((s) => {
      const booked = countBySlot[s.id] || 0;
      const full = booked >= s.capacity;
      return `<tr>
        <td>${escapeHtml(s.grades?.name || "—")}</td>
        <td>${s.school_type === "arabic" ? "Arabic" : s.school_type === "languages" ? "Languages" : "—"}</td>
        ...
        <td>${booked}</td>
        <td>${s.whatsapp_group_link ? `<span class="status-pill confirmed">Set</span>` : `<span class="status-pill cancelled">Not set</span>`}</td>
        ...
        <td class="row-actions">
          ...
          <button class="btn-tiny" onclick="editSlotWhatsappLink('${s.id}', '${escapeHtml(s.whatsapp_group_link || "").replace(/'/g, "&#39;")}')">WhatsApp Link</button>
...
// Lets an admin set/update/remove the WhatsApp group invite link tied to a
// specific lesson slot. Once a student books this slot, this is the link
// they're shown to join that slot's WhatsApp group.
async function editSlotWhatsappLink(slotId, currentLink) {
  openModal(`
    <h3>WhatsApp Group Link</h3>
    <form id="editWhatsappLinkForm">
      <div class="form-group">
        ...
        <div class="form-hint">Students who book this slot will see a "Join WhatsApp Group" button with this link on their booking confirmation. Leave empty to remove.</div>
      </div>
      <div id="editWhatsappLinkError"></div>
      <div class="modal-actions">
        <button type="button" class="btn btn-secondary" onclick="closeModal()">Cancel</button>
  ...
  $("editWhatsappLinkForm").addEventListener("submit", async (e) => {
    e.preventDefault();
    const errBox = $("editWhatsappLinkError");
    errBox.innerHTML = "";
    const rawLink = $("whatsappLink").value.trim();
    ...
    const { error } = await supabaseClient
      .from("lesson_slots")
      .update({ whatsapp_group_link: rawLink || null })
      .eq("id", slotId);
...
$("btnAddSlot").addEventListener("click", async () => {
  const { data: assignments } = await supabaseClient
    .from("teacher_subjects")
    .select("id, school_type, grades(name), subjects(name), teachers(full_name)")
    .eq("active", true);
    ...
    <form id="addSlotForm">
      <div class="form-group">
        ...
        <select class="form-select" id="slotAssignment" required>
          ...
          ${(assignments || []).map((a) => `<option value="${a.id}">${escapeHtml(a.teachers?.full_name)} — ${escapeHtml(a.subjects?.name)} (${escapeHtml(a.grades?.name)}, ${a.school_type === "arabic" ? "Arabic School" : "Languages School"})</option>`).join("")}
        </select>
      ...
      <div class="form-group">
        <label class="form-label" for="slotWhatsappLink">WhatsApp Group Link <span class="optional-tag">(optional, can add later)</span></label>
        <input class="form-input" id="slotWhatsappLink" type="url" placeholder="https://chat.whatsapp.com/...">
        <div class="form-hint">Students who book this slot will see a "Join WhatsApp Group" button with this link after booking.</div>
      </div>
  ...
  $("addSlotForm").addEventListener("submit", async (e) => {
    ...
    const whatsappLink = $("slotWhatsappLink").value.trim();

    if (whatsappLink && !/^https?:\/\//i.test(whatsappLink)) {
      errBox.innerHTML = `<div class="banner banner-error">Please enter a valid WhatsApp link starting with http:// or https://.</div>`;
      return;
    }
    if (!teacherSubjectId) {
    ...
    const { data: assignment, error: assignmentError } = await supabaseClient
      .from("teacher_subjects")
      .select("grade_id, subject_id, teacher_id, school_type")
...
window.editSlotWhatsappLink = editSlotWhatsappLink;
