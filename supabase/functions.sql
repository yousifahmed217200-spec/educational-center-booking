...
as $$
    select jsonb_build_object(
        ...
        -- ONE WhatsApp link for the whole reservation, not one per booked
        -- slot -- every item in a reservation shares the same grade, and
        -- the link is the same regardless of school type.
...
-- ----------------------------------------------------------------------------
-- available_grades_for_school_type(): grades that actually have at least
-- one BOOKABLE subject for this school type (linked + has an active teacher
-- assignment + that teacher has an active slot + that slot has remaining
-- capacity). This is what the frontend uses for the grade step -- it
-- prevents showing a grade that then turns out to have "no subjects
-- available" once selected.
-- ----------------------------------------------------------------------------
create or replace function available_grades_for_school_type(p_school_type text)
returns table (id uuid, name text, display_order integer)
language sql
security definer
stable
...
grant execute on function available_grades_for_school_type(text) to anon, authenticated;
