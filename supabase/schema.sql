...
create table if not exists center_settings (
    ...
    center_name text not null default 'سنتر ديار التعليمي',
    center_tagline text not null default 'رحلتك التعليمية تبدأ من هنا. احجز حصصك بسهولة، واختر المواد والمعلمين والمواعيد التي تناسبك.',
...
create table if not exists grades (
    ...
    -- WhatsApp group invite link for this grade as a whole (one single
    -- link, regardless of school type). A student who books ANY
    -- subject/slot under this grade sees the SAME link -- there is no
    -- longer a separate link per lesson slot or per school type.
...
create table if not exists lesson_slots (
    ...
    active boolean not null default true,
    -- DEPRECATED: per-slot WhatsApp link. The WhatsApp group link now lives
    -- on grades (grades.whatsapp_group_link) since the group is shared by
    -- the whole grade, not one slot. Column kept (unused) for backward
