...
create table if not exists lesson_slots (
    ...
    active boolean not null default true,
    -- WhatsApp group invite link for this specific slot's group, set by the
    -- admin. Shown to the student right after they book this slot, so they
    -- can join the class group immediately. NULL = no group link yet.
