...
const STEPS = [
  ...
  { key: "track", label: "المدرسة", icon: "layout-grid" },
...
const TRACKS = [
  { id: "arabic", label: "مدرسة عربي", icon: "book-text" },
  { id: "languages", label: "مدرسة لغات", icon: "languages" },
];
...
const SIDE_ART_BY_STEP = {
  ...
  track: { icon: "layout-grid", caption: "أخبرنا عن نوع مدرستك" },
...
// Uses the available_grades_for_track() RPC (see migration-track-and-age.sql)
// which returns grades matching the chosen track OR grades with no track
// restriction (track IS NULL) — so existing shared grades still show up
// regardless of which school type the student picked.
async function fetchGradesForTrack(track) {
  if (dataCache.gradesByTrack[track]) return dataCache.gradesByTrack[track];
  const { data, error } = await supabaseClient.rpc("available_grades_for_track", { p_track: track });
  if (error) throw error;
  dataCache.gradesByTrack[track] = data;
  return data;
}
...
async function goToStep(index) {
  ...
  try {
    switch (step.key) {
      ...
      case "track": renderTrackStep(); break;
...
// ============================================================================
// STEP: SCHOOL TRACK (مدرسة عربي / مدرسة لغات)
// ============================================================================

function renderTrackStep() {
  $("trackStepHeading").textContent = firstName() ? `يا ${firstName()}، نوع مدرستك إيه؟` : "نوع المدرسة";
  ...
  TRACKS.forEach((track, i) => {
    ...
    const isSelected = bookingState.track === track.id;
    card.classList.toggle("is-selected", isSelected);
    card.setAttribute("aria-checked", String(isSelected));
    card.innerHTML = `
      <span class="option-card__check">${checkIconSvg()}</span>
      <span class="option-card__icon"><i data-lucide="${track.icon}"></i></span>
      <span class="option-card__body"><span class="option-card__title">${escapeHtml(track.label)}</span></span>`;
    const select = () => selectTrack(track);
...
function selectTrack(track) {
  const trackChanged = bookingState.track !== track.id;
  bookingState.track = track.id;
  if (trackChanged) {
    // A different school track can have entirely different grades/subjects
    // /teachers/slots, so clear everything downstream — same rule already
    // applied when the grade itself changes.
...
async function renderGradeStep() {
  ...
  const grades = await fetchGradesForTrack(bookingState.track);
  listEl.innerHTML = "";
...
async function renderSubjectsStep() {
  $("subjectsStepHeading").textContent = firstName() ? `ايه المواد اللي عايز تحجزها يا ${firstName()}؟` : "ايه المواد اللي عايز تحجزها؟";
...
function validateCurrentStepBeforeAdvance() {
  ...
  switch (stepKey) {
    ...
    case "track": return bookingState.track ? null : "يرجى اختيار نوع المدرسة للمتابعة.";
