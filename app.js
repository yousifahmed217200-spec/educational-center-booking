...
function renderSuccessView(result) {
  ...
  // ONE WhatsApp join button for the whole reservation (shared by the
  // student's grade + school type), shown once beneath all booked lessons --
  // not one per lesson anymore.
  const joinWrapEl = $("successWhatsappJoinWrap");
  if (joinWrapEl) {
    joinWrapEl.innerHTML = result.whatsapp_group_link
      ? `<a class="btn btn-whatsapp-join" href="${escapeHtml(result.whatsapp_group_link)}" target="_blank" rel="noopener">
          <i data-lucide="message-circle"></i>
          <span>انضم لجروب الواتساب الخاص بالصف</span>
        </a>`
      : "";
  }
  refreshIcons();
